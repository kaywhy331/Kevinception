...
function renderDashboard() {
  ...
  const flow = `<div class="kz-flow">${flowModules.map((item, index) => `${index ? '<i aria-hidden="true">→</i>' : ''}<button type="button" data-navigate="${item[0]}"><b>${item[1]}</b><span>${item[2]}</span></button>`).join('')}</div>`;
  const attention = `<ul class="kz-attention-list">${exceptions.slice(0, 8).map((item) => `<li><div><b>${escapeHtml(item.issue)}</b><small>${escapeHtml(item.record)}</small></div><span>${escapeHtml(moduleName(item.module))}</span><span>${escapeHtml(item.age)}</span><span>${status(item.status)}</span><button type="button" data-navigate="${item.module}" data-focus-record="${escapeHtml(item.record)}">${item.action}</button></li>`).join('')}</ul>`;
  const activity = `<ul class="kz-activity-list">${state.audit.slice(0, 10).map((item) => `<li><time>${escapeHtml(item.time)}</time><b>${escapeHtml(item.module)}</b><span>${escapeHtml(item.action)}</span><span>${escapeHtml(item.record)} · ${escapeHtml(item.result)}</span></li>`).join('')}</ul>`;
  ...
  workspace.innerHTML = pageHeader('Operations Dashboard', 'One connected system for purchasing, catalog, inventory, marketplaces, fulfillment, customers, and company operations.', `<button type="button" class="kz-button kz-button--primary" data-action="sync-all">Run System Sync</button>`) + summary + `<div class="kz-grid kz-grid--dashboard">${panel('Operating Flow', flow, '<span>Vendor → customer</span>')}${panel('Needs Attention', attention, `${illustrativeBadge()}<span>${exceptions.length} open</span>`)}${panel('Recent ERP Activity', activity, '<button type="button" data-navigate="settings" data-settings-tab="Audit History">View audit history</button>', 'kz-activity-panel')}${panel('Company Homebase', companyHub, '<span>Operations intranet</span>', 'kz-company-panel')}</div>`;
}

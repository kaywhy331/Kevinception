...
export function CameraRig() {
  ...
  useEffect(() => {
    const duration = motion === 'reduced' ? 0.01 : viewMode === 'transition' ? 0.9 : 0.62;
    const positionTween = gsap.to(base.current, { x: pose.position[0], y: pose.position[1], z: pose.position[2], duration, ease: 'power2.inOut', onUpdate: invalidate });
    const targetTween = gsap.to(target.current, { x: pose.target[0], y: pose.target[1], z: pose.target[2], duration, ease: 'power2.inOut', onUpdate: invalidate });
    return () => {
      positionTween.kill();
      targetTween.kill();
    };
  ...
  useFrame(() => {
    const parallax = motion === 'reduced' || viewMode === 'interface' || viewMode === 'text' ? 0 : 0.11;
    camera.position.set(base.current.x + pointer.x * parallax, base.current.y + pointer.y * parallax * 0.5, base.current.z);

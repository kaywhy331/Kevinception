...
function createHologramPoints() {
  ...
  sphere([0, 3.1, 0], 0.58, 300);
  cylinder([0, 1.78, 0], 0.76, 2.1, 560);
  cylinder([-0.96, 1.85, 0], 0.21, 1.75, 160);
  cylinder([0.96, 1.85, 0], 0.21, 1.75, 160);
  cylinder([-0.38, 0.25, 0], 0.27, 1.8, 190);
  cylinder([0.38, 0.25, 0], 0.27, 1.8, 190);
...
function HologramFigure({ active }: { active: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    material.current.opacity = 0.7 + Math.sin(clock.elapsedTime * 2.2) * 0.08;
  });
  return (
    <points ref={ref}>
      <bufferGeometry><bufferAttribute attach="attributes-position" args={[positions, 3]} /></bufferGeometry>
      <pointsMaterial ref={material} color="#b8a8ff" size={0.034} transparent opacity={active ? 0.7 : 0.18} depthWrite={false} blending={THREE.AdditiveBlending} />
    </points>
  );
}
...
export function Year2040Scene({ active, timeline }: { active: boolean; timeline: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    shards.current.rotation.y = clock.elapsedTime * 0.11;
  ...
  return (
    <group position={[config.stationX, 0, 0]}>
      ...
      <Dust center={[0, 3.0, 0]} spread={[9.2, 5.8, 7]} color="#d4c9ff" active={active} count={active ? 72 : 18} />
      <GlassPanel position={[-5.08, 2.72, 0]} size={[4.6, 4.95, 0.05]} color="#c8f6ff" opacity={active ? 0.14 : 0.05} rotation={[0, Math.PI / 2, 0]} frameColor="#9f9aac" />
      <mesh position={[-4.98, 4.85, 0]} rotation={[0, Math.PI / 2, 0]}><planeGeometry args={[3.8, 0.22]} /><meshBasicMaterial color="#bba9ff" transparent opacity={active ? 0.4 : 0.08} /></mesh>

      <GlassPanel position={[0, 3.55, -3.35]} size={[7.6, 4.5, 0.055]} color="#d7efff" opacity={active ? 0.19 : 0.08} frameColor="#aba8b4" />
      ...
      {[-3.7, 3.7].map((x, index) => (
        <group key={x} position={[x, 2.4, -2.85]}>
          ...
          <mesh position={[0, 2.28, 0]}><sphereGeometry args={[0.34, 22, 16]} /><meshStandardMaterial color={index ? '#d9cfff' : '#c8f4ef'} emissive={index ? '#a88cff' : '#65d7c7'} emissiveIntensity={active ? 0.35 : 0.06} /></mesh>
        </group>
      ))}
      ...
      <LightBar position={[-2.7, 5.55, -2.45]} length={3.0} color="#fff5df" intensity={active ? 0.9 : 0.12} />
      <LightBar position={[2.7, 5.55, -2.45]} length={3.0} color="#ddd2ff" intensity={active ? 1.0 : 0.12} />
      ...
      <Hoverable label="Contact Kevin Echo" onClick={activate}>
        <group position={[0, 0.65, 0]}>
          <mesh position={[0, -0.34, 0]} castShadow receiveShadow><cylinderGeometry args={[1.65, 2.05, 0.3, 64]} /><meshStandardMaterial color="#d8d3df" roughness={0.28} metalness={0.12} /></mesh>
          <mesh position={[0, -0.17, 0]}><cylinderGeometry args={[1.4, 1.62, 0.12, 64]} /><meshPhysicalMaterial color="#cbbcff" transparent opacity={0.35} roughness={0.08} clearcoat={1} /></mesh>
          <HologramFigure active={active} />
          {active && <pointLight position={[0, 1.8, 0]} color="#b7a1ff" intensity={6.2} distance={8} decay={2} />}
        </group>
      </Hoverable>
          ...
          return (
            <Hoverable key={color} label={`Memory shard ${index + 1}`} onClick={() => { if (index === 0) discover('signal-fragment', '2040'); if (index === 5) discover('next-layer-message', '2040'); }}>
              <mesh position={[Math.cos(angle) * 2.85, 0.25 + Math.sin(index) * 0.5, Math.sin(angle) * 1.9]} rotation={[angle * 0.3, angle, angle * 0.2]} castShadow>
                ...
                <meshStandardMaterial color={color} emissive={color} emissiveIntensity={active ? 1.05 : 0.14} transparent opacity={0.84} />
              </mesh>
            </Hoverable>
          );
        })}
      ...
      <Hoverable label="Thought interpreter" onClick={activate}>
        <group position={[0, 0.55, 2.75]}>
          ...
          <GlassPanel position={[0, 0.38, -0.1]} size={[2.9, 0.58, 0.04]} color="#d9cfff" opacity={active ? 0.22 : 0.07} rotation={[-0.28, 0, 0]} frameColor="#aaa3b3" />
          {[-0.85, 0, 0.85].map((x, index) => <mesh key={x} position={[x, 0.05, 0.58]}><sphereGeometry args={[0.12, 18, 18]} /><meshStandardMaterial color={['#75d9ff', '#b8a8ff', '#86e6ba'][index]} emissive={['#2b8fab', '#6548c3', '#2a9e70'][index]} emissiveIntensity={active ? 0.6 : 0.08} /></mesh>)}
        </group>
      </Hoverable>
      ...
      <spotLight position={[0, 5.8, 2.6]} target-position={[0, 1.8, 0]} color="#fff6e7" intensity={active ? 4.5 : 0.42} distance={15} angle={0.72} penumbra={0.72} castShadow={active} />
      {active && <pointLight position={[0, 4.8, 2]} color="#b7a1ff" intensity={3.8} distance={12} decay={2} />}
      {active && <pointLight position={[-4.2, 3.0, 0]} color="#64d9ff" intensity={1.7} distance={8} decay={2} />}
    </group>
  );
}

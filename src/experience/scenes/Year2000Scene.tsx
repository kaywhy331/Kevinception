...
export function Year2000Scene({ active, timeline }: { active: boolean; timeline: boolean }) {
  ...
  useFrame(({ clock }) => {
    if (!modemLights.current || !power) return;
    modemLights.current.children.forEach((child, index) => {
      const material = (child as THREE.Mesh).material as THREE.MeshStandardMaterial;
      const online = active && Math.sin(clock.elapsedTime * (2.6 + index * 0.45) + index) > 0.12;
      material.emissiveIntensity = online ? 1.35 : active ? 0.22 : 0.08;
    });
  });
  ...
  return (
    <group position={[config.stationX, 0, 0]}>
      <RoomShell
        ...
        floorRoughness={0.94}
      />
      <Dust center={[0, 2.5, 0]} spread={[9.2, 5.5, 7]} color="#9fd7ff" active={active} count={78} />
      ...
      <group position={[0, 4.2, -3.33]}>
        <RoundedBox args={[3.55, 1.35, 0.12]} radius={0.06} smoothness={2} castShadow>
          <meshStandardMaterial color="#6e5945" roughness={0.95} />
        </RoundedBox>
        {[
          [-1.1, 0.25, '#d9c97e'], [-0.45, -0.22, '#99c5d9'], [0.25, 0.22, '#d995a7'], [1.05, -0.16, '#c4d39a']
        ].map(([x, y, color], index) => (
      ...
      <group position={[-0.65, 3.24, 0.2]}>
        <RoundedBox args={[5.45, 4.25, 2.48]} radius={0.36} smoothness={5} castShadow>
          <meshStandardMaterial color="#bcb7a6" roughness={0.63} />
        </RoundedBox>
        ...
        <group position={[0, -1.9, 1.28]}>
          ...
          <Hoverable label="CRT power" onClick={() => setPower((value) => !value)}>
            <mesh position={[1.82, 0, 0]}>
              ...
              <meshStandardMaterial color={power ? '#79e690' : '#604f4f'} emissive={power ? '#36a953' : '#270000'} emissiveIntensity={0.9} />
            </mesh>
          </Hoverable>
        </group>
      </group>
      ...
      <group position={[3.35, 2.35, -0.18]}>
        <RoundedBox args={[1.75, 3.72, 2.4]} radius={0.13} smoothness={3} castShadow>
          <meshStandardMaterial color="#aaa594" roughness={0.7} />
        </RoundedBox>
      ...
      <group position={[-2.15, 1.43, 1.55]}>
        <RoundedBox args={[3.2, 0.26, 1.08]} radius={0.08} smoothness={3} castShadow>
          <meshStandardMaterial color="#b4af9e" roughness={0.69} />
        </RoundedBox>
      ...
      <group position={[0.2, 1.37, 1.72]}>
        <RoundedBox args={[1.05, 0.22, 0.78]} radius={0.24} smoothness={4} castShadow>
          <meshStandardMaterial color="#aaa697" roughness={0.65} />
        </RoundedBox>
      ...
      <group position={[1.65, 1.43, 1.55]}>
        <RoundedBox args={[1.95, 0.45, 1.12]} radius={0.1} smoothness={3} castShadow>
          <meshStandardMaterial color="#343941" roughness={0.5} />
        </RoundedBox>
        <group ref={modemLights} position={[0.48, 0.26, 0.58]}>
          {[0, 1, 2, 3].map((index) => <mesh key={index} position={[-0.5 + index * 0.3, 0, 0]}><sphereGeometry args={[0.058, 12, 12]} /><meshStandardMaterial color="#6cf08c" emissive="#31c75b" emissiveIntensity={0.2} /></mesh>)}
        </group>
      </group>
      ...
      <group position={[-3.72, 2.45, -0.4]}>
        {[-1, 1].map((side) => (
          <group key={side} position={[0, side * 0.8, 0]}>
            <RoundedBox args={[0.72, 1.25, 0.65]} radius={0.11} smoothness={3} castShadow>
              <meshStandardMaterial color="#24272d" roughness={0.54} />
            </RoundedBox>
      ...
      <spotLight position={[-0.4, 5.8, 3.2]} target-position={[-0.4, 2.2, 0]} color="#d7edff" intensity={active ? 5.8 : 1.0} distance={15} angle={0.58} penumbra={0.62} castShadow={active} />
      <pointLight position={[-0.6, 3.8, 2.1]} color="#5bbcff" intensity={active && power ? 5.2 : 0.42} distance={9} decay={2} />
    </group>
  );
}

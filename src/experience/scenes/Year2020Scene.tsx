...
export function Year2020Scene({ active, timeline }: { active: boolean; timeline: boolean }) {
  ...
  const reactionData = useMemo(() => Array.from({ length: 10 }, (_, index) => ({
    x: -0.75 + (index % 5) * 0.36,
    y: (index % 7) * 0.24,
    z: -0.12 * (index % 3),
    phase: index * 0.7
  ...
  return (
    <group position={[config.stationX, 0, 0]}>
      <RoomShell
        ...
        floorRoughness={0.88}
      />
      <Dust center={[0, 2.6, 0]} spread={[9.2, 5.6, 7]} color="#ff8fb0" active={active} count={active ? 56 : 18} />
      ...
      <LightBar position={[-3.9, 5.5, -2.5]} length={2.4} color="#4fcfff" intensity={active ? 1.05 : 0.12} rotation={[0, 0.15, 0]} />
      <LightBar position={[3.9, 5.5, -2.5]} length={2.4} color="#ff4f91" intensity={active ? 1.15 : 0.12} rotation={[0, -0.15, 0]} />
      ...
      <Hoverable label="Toggle ring light" onClick={() => setRingOn((value) => !value)}>
        <group position={[-0.35, 3.0, -0.1]}>
          <mesh>
            ...
            <meshStandardMaterial color={ringOn ? '#fff5e6' : '#302d33'} emissive={ringOn ? '#fff0d2' : '#000000'} emissiveIntensity={ringOn && active ? 1.55 : 0} />
          </mesh>
      ...
      <group position={[2.85, 1.74, 0.72]} rotation={[0.03, -0.28, 0]}>
        ...
        <group position={[0, 1.03, -0.86]} rotation={[-0.07, 0, 0]}>
          ...
          <mesh position={[0, 0, 0.11]}><planeGeometry args={[2.83, 1.75]} /><meshStandardMaterial color="#111827" emissive="#31527c" emissiveIntensity={active ? 0.35 : 0.08} /></mesh>
      ...
      <group ref={reactions} position={[1.15, 0, -0.55]} visible={active}>
        {reactionData.map((item, index) => (
          <mesh key={index} position={[item.x, 1.3 + item.y, item.z]}>
            ...
            <meshStandardMaterial color={index % 2 ? '#53dcff' : '#ff5c8a'} emissive={index % 2 ? '#1b8ea9' : '#c92a62'} emissiveIntensity={0.9} />
          </mesh>
        ))}
      </group>
      ...
      <pointLight position={[-0.4, 3.0, 1.8]} color="#fff0dc" intensity={ringOn && active ? 7.2 : 0.2} distance={12} decay={2} />
      <pointLight position={[0, 4.7, 2.8]} color="#ff4f91" intensity={active ? 3.2 : 0.18} distance={11} decay={2} />
      <pointLight position={[2.6, 3.5, 1.8]} color="#4fcfff" intensity={active ? 2.3 : 0.15} distance={9} decay={2} />
    </group>
  );
}

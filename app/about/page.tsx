  ...
  return (
    <SiteChrome>
      <article id="main-content" className="about-page">
        <header className="simple-hero"><p className="eyebrow">About Kevin</p><h1>{narrativeSite.masterStatement}</h1><p className="lead">{kevinOriginNarrative.origin}</p></header>
        ...
        <section className="about-timeline"><p className="eyebrow">Six digital eras</p><h2>The interfaces changed. The pattern compounded.</h2><div>{YEAR_ORDER.map((year) => { const chapter = eraConfigs[year]; return <Link key={year} href={`/experience/${year}/`}><b>{year}</b><span>{chapter.chapterName}</span><small>{chapter.experienceName}</small></Link>; })}</div></section>
        <section className="final-cta"><h2>{narrativeSite.closingStatement}</h2><div className="button-row"><Link className="primary-action" href="/contact/">Start a conversation</Link><Link className="secondary-action" href="/work/">View work</Link></div></section>
      </article>
    </SiteChrome>
  );
}
